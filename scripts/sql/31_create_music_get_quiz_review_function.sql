-- Create music_get_quiz_review function for music theory mode quizzes
-- This function returns detailed quiz review data for completed quizzes only

CREATE OR REPLACE FUNCTION music_get_quiz_review(
  target_quiz_id UUID
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Users can only access their own quiz reviews through existing RLS policies on music_quiz table

-- Add comment
COMMENT ON FUNCTION music_get_quiz_review(UUID) IS 'Returns detailed quiz review data for completed music theory mode quizzes only';