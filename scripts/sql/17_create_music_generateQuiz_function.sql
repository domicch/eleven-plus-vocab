...
BEGIN
  ...
  FOR i IN 1 .. array_length(v_word_ids, 1) LOOP
    ...
    -- Generate question using existing music_generatequizquestion function
    SELECT music_generatequizquestion(v_word_id) INTO v_question_data;
    
    -- Check if question generation was successful
    IF v_question_data IS NULL OR v_question_data ? 'error' THEN
      RETURN jsonb_build_object(
        'error', 
        'Failed to generate question for music word ID ' || v_word_id
      );
    END IF;
...
GRANT EXECUTE ON FUNCTION music_generatequiz(UUID, INTEGER) TO authenticated;
...
COMMENT ON FUNCTION music_generatequiz(UUID, INTEGER) IS 
'Generates a complete music theory quiz with specified number of questions for a user.
Parameters:
...
and using the music_generatequizquestion function for each word.';
