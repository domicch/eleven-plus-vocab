// Tests for music_get_quiz_review database function (music mode)
const { createClient } = require('@supabase/supabase-js');

describe('music_get_quiz_review Database Function (Music Mode)', () => {
  let supabase;
  let testUser;
  let completedQuizId;
  ...
  beforeEach(async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const { error: submitError } = await supabase.rpc('music_submitquizanswer', {
  ...
  describe('Basic Functionality', () => {
    test('should return quiz review data for completed quiz', async () => {
      const { data, error } = await supabase.rpc('music_get_quiz_review', {
    ...
    test('should include all question details in review', async () => {
      ...
      data.questions.forEach((question, index) => {
        ...
        if (question.question_type === 'word_to_definition' || question.question_type === 'image_to_word') {
        ...
        } else if (question.question_type === 'music_facts') {
  ...
  describe('Access Control and Validation', () => {
    ...
    test('should return error for active (incomplete) quiz', async () => {
      ...
      const { data: newQuizResult, error: newQuizError } = await supabase.rpc('music_generatequiz', {
        user_id: testUser.id,
        question_count: 2
      });
  ...
  describe('Data Integrity', () => {
    ...
    test('should handle ultimate quiz mode', async () => {
      ...
      const { data: ultimateQuizResult, error: ultimateQuizError } = await supabase.rpc('music_generatequiz_ultimate', {
        user_id: testUser.id
      });
    ...
    test('should handle mixed question types', async () => {
      ...
      const { data: mixedQuizResult, error: mixedQuizError } = await supabase.rpc('music_generatequiz', {
        user_id: testUser.id,
        question_count: 6,
      ...
      expect(mixedQuizError).toBeNull();
      expect(mixedQuizResult).toBeDefined();
      expect(mixedQuizResult.quiz_id).toBeDefined();

      // Complete all questions
      for (let i = 0; i < 6; i++) {
        await supabase.rpc('music_submitquizanswer', {
          quiz_id: mixedQuizResult.quiz_id,
          question_index: i,
          selected_answer_index: 0
        });
      }
      ...
      const { data, error } = await supabase.rpc('music_get_quiz_review', {
        target_quiz_id: mixedQuizResult.quiz_id
      });

      expect(error).toBeNull();
      expect(data.questions).toHaveLength(6);
      
      // Verify different question types are handled correctly
      const questionTypes = data.questions.map(q => q.question_type);
      expect(questionTypes).toContain('word_to_definition');
      
      // Each question should have the appropriate structure
      data.questions.forEach(question => {
        expect(['word_to_definition', 'image_to_word', 'music_facts']).toContain(question.question_type);
      });
    });
  });
});